...
#define DIGEST_CTX(OPERATION_HANDLE) ((hash_state *)OPERATION_HANDLE->dig_ctx)
#define RSA_key(OPERATION_HANDLE) ((rsa_key *)(OPERATION_HANDLE->key.key))
#define DSA_key(OPERATION_HANDLE) ((dsa_key *)(OPERATION_HANDLE->key.key))
#define DH_key(OPERATION_HANDLE) ((dh_key *)(OPERATION_HANDLE->key.key))
#define EC_key(OPERATION_HANDLE) ((ecc_key *)(OPERATION_HANDLE->key.key))
#define SYM_ctx(OPERATION_HANDLE) ((void *)(OPERATION_HANDLE->key.ctx))
#define SYM_key(OPERATION_HANDLE) (OPERATION_HANDLE->key.key)
#define SYM_key_len(OPERATION_HANDLE) (OPERATION_HANDLE->key.key_len)
#define HMAC_ctx(OPERATION_HANDLE) ((hmac_state *)(OPERATION_HANDLE->key.ctx))

#ifdef XXX
#define CMAC_ctx(OPERATION_HANDLE) ((void *)(OPERATION_HANDLE->key.ctx))
#endif
...
static bool cpy_key_comp_to_bn(void **bn, uint32_t attrID, TEE_ObjectHandle key)
{
...
static void set_symmetric_padding(uint32_t algorithm, void *ctx)
{
	switch (algorithm) {
	case TEE_ALG_AES_ECB_NOPAD:
	case TEE_ALG_AES_CBC_NOPAD:
...
static const void *load_evp_sym_cipher(TEE_OperationHandle operation)
{
...
static const hash_state *load_evp_asym_hash(TEE_OperationHandle operation)
{
	switch (operation->operation_info.algorithm) {
	case TEE_ALG_RSA_NOPAD:
		return NULL;
	...
	if (key->objectInfo.objectType == TEE_TYPE_RSA_KEYPAIR) {
		...
		if (get_attr_by_ID(key, TEE_ATTR_RSA_PRIME1) ||
			...
			if (!cpy_key_comp_to_bn(&RSA_key(operation)->p, TEE_ATTR_RSA_PRIME1, key) ||
			    ...
			    !cpy_key_comp_to_bn(&RSA_key(operation)->dP, TEE_ATTR_RSA_EXPONENT1, key) ||
			    !cpy_key_comp_to_bn(&RSA_key(operation)->dQ, TEE_ATTR_RSA_EXPONENT2, key) ||
			    !cpy_key_comp_to_bn(&RSA_key(operation)->qP, TEE_ATTR_RSA_COEFFICIENT, key)) {
...
static uint32_t rsa_msg_max_len(TEE_OperationHandle operation, const hash_state *hash)
{
	switch (operation->operation_info.algorithm) {
	case TEE_ALG_RSAES_PKCS1_V1_5:
...
static TEE_Result check_rsa_bufs_len(TEE_OperationHandle operation, const hash_state *hash,
...
static bool add_rsa_cipher_padding(TEE_OperationHandle operation, void *srcData, uint32_t srcLen,
				   ...
				   uint32_t paramCount, const hash_state *hash)
{
...
static TEE_Result rsa_op_generic_pre_checks_and_setup(TEE_OperationHandle operation,
						      const hash_state **hash, void *srcData,
...
static TEE_Result rsa_sign_ver_generic_checks_and_setup(TEE_OperationHandle operation, void *digest,
							...
							unsigned char **rsa_mod_len_buf,
							uint32_t *rsa_mod_len, const hash_state **hash,
							uint32_t *NID_value)
{
	if (!digest || !signature) {
	...
	*NID_value = get_openssl_NID_value(operation->operation_info.algorithm);
	...
	if (dig_len != int2uint32(hash_state_size(*hash))) {
...
#ifdef XXX
		if (*salt == -1)
			*salt = hash_state_size(*hash);

		if (uint322int(*rsa_mod_len) < (hash_state_size(*hash) + *salt + 2)) {
			OT_LOG(LOG_ERR, "rsa_sign_ver_generic_checks_and_setup: "
					"Dig too big for RSA key\n");
			TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
		}
...
static bool add_rsa_signature_padding(TEE_OperationHandle operation, void *EM, void *mHash,
				      uint32_t mHash_len, const hash_state *hash, int salt)
{
	switch (operation->operation_info.algorithm) {
	case TEE_ALG_RSASSA_PKCS1_V1_5_MD5:
	case TEE_ALG_RSASSA_PKCS1_V1_5_SHA1:
...
static TEE_Result malloc_and_cpy_dsa_key(TEE_OperationHandle operation, TEE_ObjectHandle key)
{
	...
	if (!cpy_key_comp_to_bn(&DSA_key(operation)->p, TEE_ATTR_DSA_SUBPRIME, key) ||
	    ...
	    !cpy_key_comp_to_bn(&DSA_key(operation)->y, TEE_ATTR_DSA_PUBLIC_VALUE, key) ||
	...
	if (key->objectInfo.objectType == TEE_TYPE_DSA_KEYPAIR &&
	    !cpy_key_comp_to_bn(&DSA_key(operation)->x, TEE_ATTR_DSA_PRIVATE_VALUE, key)) {
...
#ifdef XXX
	if (!cpy_key_comp_to_bn(&DH_key(operation)->idx, TEE_ATTR_DH_PRIME, key) ||
	    !cpy_key_comp_to_bn(&DH_key(operation)->type, TEE_ATTR_DH_BASE, key) ||
	    !cpy_key_comp_to_bn(&DH_key(operation)->x, TEE_ATTR_DH_PUBLIC_VALUE, key) ||
	    !cpy_key_comp_to_bn(&DH_key(operation)->y, TEE_ATTR_DH_PRIVATE_VALUE, key)) {
...
static const void *load_evp_mac_cipher(TEE_OperationHandle operation)
{
	uint32_t algorithm = operation->operation_info.algorithm;
	uint32_t key_size = operation->operation_info.keySize;
...
static const hash_state *load_evp_mac_hash(TEE_OperationHandle operation)
{
	switch (operation->operation_info.algorithm) {
#ifdef XXX
	case TEE_ALG_DES_CBC_MAC_NOPAD:
...
static int max_mac_len(TEE_OperationHandle operation)
{
	...
	case TEE_ALG_HMAC_SHA384:
	case TEE_ALG_HMAC_SHA512:
		return hash_state_size(load_evp_mac_hash(operation));
#endif
...
static TEE_Result init_operation_meta_info(TEE_OperationHandle operation, uint32_t algorithm)
{
	...
	case TEE_ALG_HMAC_SHA384:
	case TEE_ALG_HMAC_SHA512:
		operation->key.ctx = (hmac_state *)TEE_Malloc(sizeof(hmac_state), 0);
...
void TEE_FreeOperation(TEE_OperationHandle operation)
{
	...
	if (operation->operation_info.operationClass == TEE_OPERATION_DIGEST)
		hash_state_CTX_destroy(DIGEST_CTX(operation));
#endif
...
#ifdef XXX
		hash_state_CTX_destroy(DIGEST_CTX(operation)); /* No documented return value */
#endif
...
#ifdef XXX
		if (hash_state_CTX_copy_ex(dstOperation->dig_ctx, srcOperation->dig_ctx) != 1) {
